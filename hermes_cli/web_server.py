...
=======
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("claude_code_cli", "openai_codex", ...)
...
=======
    # claude-code uses the external Claude Code CLI's credentials at
    # ~/.claude/.credentials.json. We don't touch that file — it's owned by
    # the Claude Code CLI. A disconnect just clears any cached pool entry so
    # Hermes stops surfacing it; users can re-auth via `claude /login` or
    # `claude setup-token` to bring it back.
